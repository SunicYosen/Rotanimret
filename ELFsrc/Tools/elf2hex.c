...
int main(int argc, char *argv[])
{
	...
	for (phdr = (Elf64_Phdr *)pdata, i = 0; i < ehdr.e_phnum; i++) {
		if (
			phdr->p_type == PT_LOAD &&
			phdr->p_vaddr <= start &&
	...
	data = (char *)malloc(len = stop - start);    //New the space for we want
	if (data == NULL) {
		perror("malloc");
		exit(1);
	}
	...
	n = 0;
	while (n < len) {         // You can change the form of output by change here.
		for (i = 0; n < len && i < N; i++) {
			printf(",0x%2.2x", data[n] & 0xff);
			++n;
		}
		printf("\n");
	}
